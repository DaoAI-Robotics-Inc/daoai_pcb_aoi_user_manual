...
- **需要在检测中对齐** : 是否启用该元件的对齐功能, 默认开启。启用后，系统会在检测时自动微调该元件的检测框的位置，确保其与模板位置一致。
- **检测允许偏移** ： 当该元件的偏移超过设定值时，将不会对齐该元件，可根据实际情况调整该阈值。需要注意的是，阈值设置过高可能会忽略元件的实际偏移，导致检测不到偏移问题。
- **检测允许旋转** ： 当该元件的旋转超过设定值时，将不会对齐该元件，可根据实际情况调整该阈值。需要注意的是，阈值设置过高可能会忽略元件的实际旋转，导致检测不到旋转问题。
...
- **自动调整元件** ：在拼版场景中，子板上的元件可能因位置偏移而与主板不完全一致。启用此功能后，系统会根据主板上对应元件的位置，自动调整当前子板元件的检测框，确保检测区域的准确性。
