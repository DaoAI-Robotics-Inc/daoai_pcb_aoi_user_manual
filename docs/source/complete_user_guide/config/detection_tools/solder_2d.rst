...
颜色范围支持 **三角形抽色** 配置：在 HSV 颜色空间中，用一个三角形圈出目标颜色簇。三角形由三个顶点定义，分别对应中心色、起始色与结束色，比传统的"矩形" HSV 范围更紧凑、误抽其他颜色更少。可在 HSV 颜色块上直接拖动三个顶点调整范围。
...
参数面板顶部提供 **抽色二值化预览** 切换，开启后可在画布上实时叠加显示当前颜色范围抽到的像素分布，便于调参时直观确认抽色效果。
