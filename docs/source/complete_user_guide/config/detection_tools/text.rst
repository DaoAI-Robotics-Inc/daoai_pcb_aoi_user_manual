...
.. tip::
   ...
   - **OCR vs OCV 怎么选**：OCR 识别具体字符内容（需要验证字符是什么），OCV 仅验证字符外观与参考是否一致（不识别具体内容）。普通情况下 OCR 更稳定、误报更少；字符模糊或字体特殊时优先用 OCV。
   - **OCR 检测框画多大**：建议比文字宽约 **20% 左右** 即可，过大容易识别到框外文字。
   - **白底黑字 / 黄底黑字识别失败**：当前部分背景色组合 OCR 模型支持有限，可关闭 OCR、改用 OCV 比对字符外观。
   - **手动添加的文本检测框识别不出**：先确认字符方向与 OCR 框内方向一致，必要时启用 **双向检测 / 启用旋转** 选项。
