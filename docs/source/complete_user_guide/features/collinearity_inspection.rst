...
检测运行后，共线性组的判定结果会出现在 :ref:`检测与反馈` 的复检页面中。共线性相关的不良以错误类型 **共线性检测** 列出，特征错误面板逐条展示该组每个成员的以下信息：

- **组判定** — 该共线性组的整体判定，取值为 **通过**、**失败** 或 **无法判定**。
- **位置（行 X, 列 Y）** — 当前成员在网格中的行列坐标。
- **列方向偏差** / **行方向偏差** / **角度偏差** — 成员实测位置相对拟合点阵预测位置的偏差；任一项超出组设定的对应阈值即判为失败。
- **成员已丢弃（无对齐）** — 某成员在本次检测中未能完成对齐时，该成员被排除在拟合之外并如此标注。

当组判定为 **无法判定** 时，面板会一并显示 **原因**，可能为：

- **组尚未拟合 — 请在示教页运行拟合。**
- **可对齐的成员数不足。**
- **存储的拟合数据格式异常。**

.. screenshot-todo: images/collinearity_review_result.png — 复检页面 特征错误面板中的共线性检测结果：组判定（通过/失败/无法判定）、位置（行/列）、列方向偏差/行方向偏差/角度偏差、以及无法判定时的原因
