...
在产品编程页面的 **设置（Recipe）** 标签页中，**检测设置** 用于配置拼板 **子板（sub-PCB）** 的判废逻辑，决定每个子板被判为好板还是坏板：

- 使用 NG 元件比例（比例模式）：设置 **失败比例阈值** ``0.0–1.0``；当子板的 NG 元件比例超过该阈值时，该子板判为坏板。
- 使用坏板标记元件（坏板标记模式）：基于在元件上定义的坏板标记判定（见 :ref:`检测控制与特殊标记`）。**组合逻辑** 可选：

   - **任一 NG**：任一坏板标记判为坏 ⇒ 子板坏。
   - **全部 NG**：全部坏板标记判为坏 ⇒ 子板坏。

- 若两种模式均不启用，则子板 **仅人工复检**，不会被自动判定。
...
**AI 检测 ROI（AI-detect ROI）**：框选一块区域后，可点击 “AI检测ROI”，选择默认检测类型（本体 / 焊料 / IC 引脚等），由 AI 自动识别并生成 ROI（检测框），减少逐个手动绘制的工作量。
