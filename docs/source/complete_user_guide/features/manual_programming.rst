...
.. note::
   3D SMT 模式下，工具栏还包含 **裁剪3D视图（Crop 3D View，快捷键 P）**，用于框选点云区域。本手册仅涵盖 2D 模式，不介绍该工具。

各检测工具的具体检测参数说明见 :ref:`检测工具参考` 中各工具页面。

**AI 检测 ROI（AI-detect ROI）**

框选一块区域后，可点击工具栏下方的 **AI检测ROI** 按钮，选择默认检测类型（本体、焊锡、IC 引脚等），由 AI 自动识别并在所选区域内生成检测框（ROI），减少逐个手动绘制的工作量。
...
- **组合为坏板标记（Combine into badmark）**：仅当特征为未分组状态（未归属任何元件）时显示。将选中的未分组特征直接组合为一个坏板标记元件，无需弹窗，自动命名为 ``BADMARK_<N>``。
- **删除（Remove）**
...
- **组合为坏板标记（Combine into badmark）**：在满足条件时显示（所选特征不含跨子板实例且不属于共享面板区域）。
- **复制（Copy）（N）**
- **删除（Delete）（N）**
...
.. note::
   **合并为坏板标记** 功能仅对未分组特征生效。已属于某元件的特征需在 :ref:`元件配置` 的元件信息面板中通过"元件类型"选择器将其设为坏板标记。
