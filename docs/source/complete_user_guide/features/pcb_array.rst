...
.. list-table:: 二维码与拼板组合的序列号取值
   ...
   * - ④
     - 有拼板
     - 二维码在每个拼板内（每个拼板各一个）
     - 自动生成（v4.1-652/653/654 之后改为任意拼板二维码）
     - 各拼板内二维码
   * - ⑤
     - 有拼板
     - 拼板内各有二维码 + 拼板外还有一个
...
.. tip::
   ...
   **二维码读取常见问题：**
   ...
   - **MES 报告中 MainSN / PanelSN 不符合预期**：对照上方场景表确认当前产品属于哪种场景。场景 ④ 中 MainSN 为随机值属正常现象（v4.1-652 之后会自动取任意拼板的 PanelSN 作为 MainSN）。如需 MainSN 为固定值，请在拼板区域外额外添加一个整板级二维码框，使产品落到场景 ⑤。
