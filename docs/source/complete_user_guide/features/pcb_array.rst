...
.. note::
   本页描述的"管理子板模板 → 自动检测 → 自动调整拼板"流程也可在创建产品时自动完成：在 :ref:`自动编程` 配置中开启 **启用拼板** 并填写行 / 列后，系统会在自动编程结束后自动构建拼板阵列。本页流程用于手动拼板，或自动检测失败后的回退处理。
...
序列号（MainSN / PanelSN）的生成与赋值规则取决于产品 **是否配置了拼板**，以及 **二维码识别框的位置**。下表覆盖 5 种常见场景；请按实际配置对照排查 MES 报告中 MainSN / PanelSN 的取值是否符合预期。

.. list-table:: 二维码与拼板组合的序列号取值
   :header-rows: 1
   :widths: 8 16 28 24 24
...
.. tip::
   ...
   **二维码读取常见问题：**
   ...
   - **二维码读取失败，序列号变成自动生成值**：检查二维码识别框是否完整覆盖条码区域；二维码本身是否清晰、无遮挡、无污损；多拼板时若部分拼板失败，检查拼板对齐是否偏移。
   - **MES 报告中 MainSN / PanelSN 不符合预期**：对照上方场景表确认当前产品属于哪种场景。场景 ④ 中 MainSN 会自动取任意拼板的 PanelSN。如需 MainSN 为固定值，请在拼板区域外额外添加一个整板级二维码框，使产品落到场景 ⑤。
   - **阴阳板 / 鸳鸯板拼板**：智能拼板暂不支持此类布局，可使用 **手动复制粘贴拼板** 方式（旧拼板流程仍保留可用）。
