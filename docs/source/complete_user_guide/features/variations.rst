...
替代料是指同一元件位置上，被系统认可的多种合法外观形态。当检测流程将某一外观实际合格的元件误判为 NG 时，操作员可在复检页面将其标记为替代料，从而让系统在今后的检测中接受该外观。典型场景：同一丝印位置更换了不同封装或不同供应商的物料，但旧训练图像尚未涵盖新外观。

替代料与主元件共享同一元件位置，在模板编辑器中以独立行显示，可单独查看其特征。创建替代料后需重新训练模型，才能在后续检测任务中生效。
...
在复检页面选中误判为 NG 的元件后，点击 **...** → **设置为替代料**，弹出 **添加为替代料** 对话框。
...
选择 **沿用当前元件图像** 后，对话框进入第二步，显示批量同步选项。操作员可选择同步范围：
...
- **料号输入框 + 保存按钮** ：修改该替代料组的料号，点击保存后将更新所有共享该料号的相关元件。

- **封装号输入框 + 保存按钮** ：修改该替代料组的封装号，点击保存后将更新所有共享该封装号的相关元件。
...
.. note::
   ...
   - **批量同步只能选择一种范围（料号或封装号，二选一或不选）。** 若同一元件的料号或封装号字段为空，对应的同步选项将被禁用。
   - **替代料在模板编辑器中以独立行显示**，与原始主元件分开，不影响主元件的特征配置。
   - **本页面仅描述离线复检流程。** 联机多后端（ReviewOnline）场景及 MES 集成不在本页范围之内。

.. warning::

   **拼板场景下的替代料默认作用于所有子板。** 添加一次替代料后，所有子板上同一位置的元件都会获得这条替代料；**无需也不应** 重复在不同子板的相同位置上各添加一遍。重复添加会显著增加每次检测的耗时（每条替代料都是一组独立的推理参考），并可能拖累整体节拍。
