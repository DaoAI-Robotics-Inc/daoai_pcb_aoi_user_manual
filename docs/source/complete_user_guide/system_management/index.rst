...
本章节介绍系统级通用配置。点击右上角 **齿轮图标** 进入 **设置**，左侧分为 **团队**、**管理模型**、**系统设置**（含 语言、主机、系统配置、捕获模块、模型更新器、编程默认参数、错误类型翻译、校准、关于、日志）以及 **登出** 等分组。
...
2. 在“当前主机”输入框中查看 / 修改后端地址（默认为 `localhost:8000`）。
...
用于自定义缺陷 / 错误类型的中英文显示标签，以及对应的 MES 厂商代码（在与 MES 系统对接时用于错误编码映射）。
...
- **错误类型**：系统内置的错误类型标识。
- **英文标签 / 中文标签**：在界面与报告中显示的名称，可按需自定义。
- **MES 厂商代码**：导出 MES 结果时使用的错误代码映射。
...
**设置** 左侧的 **管理模型** 用于查看系统中的检测模型（按 全部 / 预训练 / 已训练 筛选），并支持上传模型、设为默认、重命名、导出与删除等操作。模型训练、数据集导入 / 导出等完整流程详见 :ref:`模型管理` 章节。
