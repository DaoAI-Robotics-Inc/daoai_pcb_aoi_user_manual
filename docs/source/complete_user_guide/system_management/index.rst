...
本章节介绍系统级通用配置。点击右上角 **齿轮图标** 进入 **设置**，左侧导航栏分为以下主项：**团队**（仅 Admin 可见）、**管理模型**、**系统设置**（含 语言、主机、系统配置、全局 OCR 模糊映射、捕获模块、模型更新器、编程默认参数、错误类型翻译、标签、校准、关于、日志）以及 **登出**。
...
.. note::
   导出配置文件的选择、默认 ``default_csv`` 配置的存图路径与报告字段说明，详见 :ref:`MES 结果导出`。
...
进入路径：右上角齿轮 → **设置** → **系统设置** → **全局 OCR 模糊映射** \ （Admin / 编程员 可见）。

页面标题为"全局 OCR 模糊字符映射"。这里维护一份全线统一的字符等价表，供 **文本检测（OCR）** 的模糊匹配使用：同一分组内的字符在 OCR 文本校验时视为可互换（例如 ``O`` / ``0`` / ``Q``）。每个分组需包含 **3 个或更多** 字符，该表对所有启用了模糊匹配的产品生效。

操作说明：

- **添加分组**：新增一个等价分组。
- 在分组内点击 **+ 字符** 添加成员；每个成员只能是 **单个** 大写字母或数字（``A``–``Z`` / ``0``–``9``），输入会自动转为大写。
- **删除分组**：移除整个分组。
- 编辑完成后点击 **保存全局映射**。保存成功提示"全局 OCR 模糊映射已保存"，失败提示"保存全局 OCR 模糊映射失败"。保存时少于 2 个成员的分组会被自动丢弃。

.. warning::
   过宽的分组（例如 ``8`` ↔ ``B``）会使该混淆在全线所有文本元件上都被接受，从而在各处屏蔽此类缺陷。请保持全局映射的保守；针对特定料号的情况，请改用文本检测页 :ref:`文本检测（Text / OCR · OCV）` 的 **模糊模式（Blur Mode）** 按元件学习。

.. note::
   修改后在 **下一次检测会话启动时生效**，无需重启后端；正在运行的会话不会立即更新。

.. image:: images/global_ocr_fuzzy_map.png
   :scale: 60%
   :alt: 全局 OCR 模糊字符映射 编辑页面：分组列表与 添加分组 / 保存全局映射 按钮
