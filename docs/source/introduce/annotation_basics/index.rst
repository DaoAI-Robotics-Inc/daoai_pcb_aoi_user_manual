...
- **本体框（Body）**：框住元件主体，几乎所有元件都有的基础框，用于检测元件是否存在、是否偏移、是否错件或反向。
- **焊锡框（Solder）**：框住引脚根部的焊点与爬锡区域，检测焊锡量与形态（少锡、多锡、虚焊等）；结合光源原理，爬锡坡面在 2D 图中通常呈蓝到绿。
- **IC 引脚框（IC Lead）**：用于带引脚的 IC，由四个子区域框组成，各代表引脚的一段，如下表所示。
- **文本框 / OCR 框（Text）**：框住需要识别的字符区域（丝印、料号、批次或日期码），用于 OCR 文本检测与比对。
- **极性检测框（Polarity）**：框住元件上的极性标识（电容极性条、二极管色环、LED 缺角或标记等）；在“本体检测（2D）”中启用极性检测后，系统据此判断元件是否装反。
